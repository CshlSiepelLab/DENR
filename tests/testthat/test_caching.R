...
db_string = 'homo_sapiens-grch38.p12-ensembl_genes_96_test.txdb'


# Test cache location
test_cache_location = file.path(".","temp_cache")
...
test_that("TxDb can be cached and retrieved",{
    ...
    expect_message(save_txdb(txdb = txdb,annotation_version = 'ensembl_genes_96_test'),
                   regexp = paste0("Saving db to ",file_path))
    expect_s4_class(load_txdb(db_string),
                    class = "TxDb")
})
