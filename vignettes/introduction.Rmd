...
\code{min_transcript_length} here just for the purposes of demonstration with a synthetic
data set. Generally using transcripts that short is not reccomended).
...
tsp <- transcript_shape_profile(transcripts = gr_ds,
                                ...
                                bigwig_minus = bwm,
                                bin_size = bsize,
                                min_transcript_length = 1e3)
```

The empirical profile can be viewed with the \code{view()} function. This example looks a
little odd as we are running on synthetic data and allowing really short transcripts.
...
using the \code{inactive_transcripts} option.
